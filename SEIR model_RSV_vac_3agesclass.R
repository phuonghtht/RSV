...
E3 <- 0
I3 <- infected_seeds/(pop_size*(1/80))
R3 <- 0
S3 <- 1-E3-I3-R3 
...
params     <- c(mu = 0.0135,     # birth rate
                b = 550,         # 
                a = 0.522,        #amplitude of seasonal forcing = phi or beta1
                ...
                sigma3 = 0.6,
                alpha2 = 0.228, # Susceptibility scaling factor          
                alpha3 = 0.6               
)  
  ...
  with(as.list(c(states, params)),
       {
         ...
         beta1 <- b*(1+a*sin(2*pi*t))
         # beta2 < alpha2*beta1
         # beta3 <-alpha3*alpha2*beta1 
         
         
         # calculate state changes
         dS1 <- mu - beta1*S1*(I1+sigma2*I2+sigma3*I3) + nu*R1 - eta1*S1 
         dE1 <- beta1*S1*(I1+sigma2*I2+sigma3*I3) - delta*E1 - eta1*E1 
         dI1 <- delta*E1 - gamma*I1 - eta1*I1 
         dR1 <- gamma*I1 - nu*R1 - eta1*R1
         
         dS2 <- eta1*S1 - alpha2*beta1*S2*(I1+sigma2*I2+sigma3*I3) + nu*R2 - eta2*S2 
         dE2 <- eta1*E1 + alpha2*beta1*S2*(I1+sigma2*I2+sigma3*I3) - delta*E2 - eta2*E2
         dI2 <- eta1*I1 + delta*E2 - gamma*I2 - eta2*I2 
         dR2 <- eta1*R1 + gamma*I2 - nu*R2 - eta2*R2 
         
         dS3 <- eta2*S2 - alpha3*alpha2*beta1*S3*(I1+sigma2*I2+sigma3*I3) + nu*R3 - eta3*S3 
         dE3 <- eta2*E2 + alpha3*alpha2*beta1*S3*(I1+sigma2*I2+sigma3*I3) - delta*E3 - eta3*E3
         dI3 <- eta2*I2 + delta*E3 - gamma*I3 - eta3*I3 
         dR3 <- eta2*R2 + gamma*I3 - nu*R3 - eta3*R3 
...
# summary(out)
times_output <- seq(num_years-20,num_years,1)# burning time ## shifting peaks
out <- out[out[,1] %in% times_output,] # skip the initial time points
out[,1] <- out[,1] - min(out[,1])# rescale time points

par(mfrow = c(1,1))
matplot(out[,1], out[,c(4,8,12)], type = "l", xlab = "time", ylab = "population fraction")
legend("topright", col = 1:3, lty = 1:3, legend = c("I1","I2","I3"))
