...
pro_case_less1yrs <- 13752/63301
report.data$week <- seq(1,dim(report.data)[1])
report.data$cases.less1yrs <- round(report.data$cases*pro_case_less1yrs)
## score for initial model
# ggplot(report.data, aes(week, cases.less1yrs))+
#   geom_point()
...
plot(report.data$week,
     report.data$cases.less1yrs,
     ylim = c(0, max(report.data$cases.less1yrs)*1.5))

# plot initial model
lines(out$time,out$I1*pop_size,col=2,lwd=2)

# score for initial model
get_sum_of_squares(out$I1[1:length(report.data$cases)]*pop_size,report.data$cases.less1yrs)

# DEFINE FUNCTION TO RUN ODE WITH PARAMETER VECTOR X
x <- c(0.65, 0.65,0.65,1.99,-154) # delta[0.5,0.8], alpha[0.5,0.8], beta1[0,1], beta0[1.99]
...
get_parameter_score <- function(x) {
  ...
  # get model score
  model_score <- get_sum_of_squares(model_out$I1[length(report.data$cases)]*pop_size,report.data$cases.less1yrs)
...
plot_model_fit <- function(x){
  ...
  # get model score
  model_score <- get_sum_of_squares(model_out$I1[1:length(report.data$cases)]*pop_size,report.data$cases.less1yrs)

  # plot reference data
  plot(report.data$week,
       report.data$cases.less1yrs,
       ylim = c(0,max(report.data$cases.less1yrs)*2),
...
# num_days_exposed  <- 4  # [2,6]
opt_param_sa <- optim_sa(fun = get_parameter_score, start = c(0.6, 0.6,0.8,1.9,-200),
                         trace = FALSE,
                         lower = c(0.5, 0.5,0,1.85,-519),#
                         upper = c(0.8, 0.8,1,2.0,-100),
...
opt_param_sa
# [1]    0.64    0.66    0.85    1.91 -201.08  # 1 yr group proportion
...
par(mfrow = c(1,1))
jpeg(paste("~/Dropbox/2.Master of Epidemiology_Antwerp/year 2/thesis/data analysis/infection p=",vac.frac,".jpg"))
matplot(out[,1], out[,c(4,9)], type = "l", xlab = "time", ylab = "population fraction",
        main = paste(" Vaccination p =",vac.frac))
legend("topright", col = 1:2, lty = 1:2, legend = c("I1","I2"))
dev.off()
...
report.data <- read.csv("./RSV data/RSV_cases_time_epistat.csv")
# total cases: 63301,4yrs: 822,3yr:1794,2yr:4559 1yr:13751,0yr:42375
pro_case_less1yrs <- 13751/63301 # 0yr group
report.data$week <- seq(1,dim(report.data)[1])
report.data$cases.less1yrs <- round(report.data$cases*pro_case_less1yrs)
## score for initial model
# ggplot(report.data, aes(week, cases.less1yrs))+
...
# plot real data
par(mfrow = c(1,1))
# jpeg(paste("~/Dropbox/2.Master of Epidemiology_Antwerp/year 2/thesis/data analysis/vaccination_p",vac.frac,".jpg"))
plot(report.data$week,
     report.data$cases.less1yrs,
     xlab = "Time (weeks)",
...
out_vac <- out[,c("I1","I2")]
out_vac$time <- seq(1: dim(out)[1])
out_vac$I1.vac <- out_vac$I1*pop_size
out_vac$I2.vac <- out_vac$I2*pop_size

# write.csv(out_vac,file = paste("~/Dropbox/2.Master of Epidemiology_Antwerp/year 2/thesis/data analysis/vaccination_p",vac.frac,".csv"))

# df <- read.csv("/home/phuong/Dropbox/2.Master of Epidemiology_Antwerp/year 2/thesis/data analysis/vaccination_p 0.1 .csv")
