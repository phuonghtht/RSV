...
t_shift = 462        # optimized from fitting# this change up to what fitted set of params
t_vac = 0            # 0,52wk ## vaccination start't_vac'wks before the 1st epidemic(1st wk 2011)
vac.target = "AG1"   # "newborn" or "AG1" or "both"
...
# pop_size          <- 11176549/2 # assuming RSV surveillance data cover only 50% of total RSV
...
if(pop_size == 11176549){
  ...
  params     <- c(sigma = 7/num_days_exposed,  #latent to infectious stage
                  ...
                  nu=  0.038,                  # rat e of loss of immunity
  ...
  #sigma = 7/num_days_exposed, gamma = 7/num_days_infected
   ...
   # plot_model_fit(c(1.83,0.88,3.1,-358,0.034)) # 37842.93 => scaled reported data
  ...
  with(as.list(c(states, params)),
       {
         ...
         if(vac.str=="year-round"){
           ...
           if(t<(t_shift-t_vac)){ #t_vac=0 be default
             p <- 0
           }else {
             p <- p
           }
         ...
         if(vac.str=="seasonal"){
           ...
           if(t< (t_shift+32)){# skip 32 wks of 2011
             p <- 0
           }
           if(t>= (t_shift +32)){# start vaccination epidemic season 2011/2012
             t_week = (t-t_shift+1)%%52
         ...
         if(vac.target=="AG1"){
           # calculate state changes
           dS1 <-         mu  - beta*S1*(I1+alpha*I2)  + nu*R1                - eta1*S1 - p*e*S1
           dE1 <-               beta*S1*(I1+alpha*I2)  - sigma*E1             - eta1*E1
           dI1 <-                                        sigma*E1  - gamma*I1 - eta1*I1
           dR1 <-                                      - nu*R1     + gamma*I1 - eta1*R1 + p*e*S1
         ...
         if(vac.target=="both"){
           # calculate state changes
           dS1 <- (1-p*e)*mu  - beta*S1*(I1+alpha*I2)  + nu*R1                - eta1*S1 - p*e*S1
           dE1 <-               beta*S1*(I1+alpha*I2)  - sigma*E1             - eta1*E1
           dI1 <-                                        sigma*E1  - gamma*I1 - eta1*I1
           dR1 <-                                      - nu*R1     + gamma*I1 - eta1*R1 + p*e*S1
...
# plot initial model
lines(out$time[1:length(case_dt$cases_ag1)],out$I1[1:length(case_dt$cases_ag1)]*pop_size,col=2,lwd=2,type = "b",pch=1)
...
plot(out$time, # plot seasonal 
     ...
     main= paste("Vaccination =",p,vac.str,vac.target),col=2,type = "l",lwd=2,
     xlim = c(0,52*10),
     xaxt="none")# for later customizing
...
# sigma = 7/num_days_exposed, gamma = 7/num_days_infected
  # plot_model_fit(c(1.84,0.855,2.94,-411,0.034)) #22427.65
  # plot_model_fit(c(1.84,0.855,2.89,-412,0.034)) #25994.06
  # plot_model_fit(c(1.84,0.85,0.18,-539,0.034))  #30510.14
  # plot_model_fit(c(1.84,0.855,2.89,-412,0.033)) #25994.06

```

##Scaling report data 
Assuming only 70% RSV cases are reported.
...
par(mfrow = c(1,1))
plot(case_dt$week, # plot seasonal 
     case_dt$cases_ag1/0.7,
     xlab = "Time (weeks)",
     ylab = "Number of RSV reported",
     pch=19,col=1,
     ylim = c(0, max(case_dt$cases_ag1/0.7)*1.2),
     xaxt="none")# for later customizing
axis(1,seq(0,350,by=50))# customize x-axis

# plot initial model
lines(out$time[1:length(case_dt$cases_ag1)],out$I1[1:length(case_dt$cases_ag1)]*pop_size,col=2,lwd=2,type = "b",pch=1)
legend(309,1710, legend = c("observed","model fit"), col = c(1,2),lty=1:2,pch=c(19,1))
...
################################# #
# Scaling observed data 70%
################################# #
  # #sigma = 7/num_days_exposed, gamma = 7/num_days_infected
   # plot_model_fit(c(1.83,0.88,3.1,-358,0.034)) # 37842.93
   # plot_model_fit(c(1.853,0.935,3.1,-306,0.036)) #27397.85
   # plot_model_fit(c(1.815,0.852,3.3,-357,0.035)) #37859.64
   # plot_model_fit(c(1.829,0.886,3.1,-462,0.034))# 34028.22
   # plot_model_fit(c(1.82,0.88,3.1,-462,0.039)) # 23207.43
   # plot_model_fit(c(1.82,0.88,3.1,-462,0.036)) # 24552.02
